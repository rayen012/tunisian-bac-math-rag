...
def _build_existing_tex_set(bucket: storage.Bucket, prefix: str) -> set:
    """Pre-fetch all .tex blob names under prefix for fast lookup."""
...
def iter_pending_blobs(
    ...
    """Yield blobs that need digitization (no companion .tex yet)."""
    for blob in bucket.list_blobs(prefix=prefix):
...
7. IGNORE COMPLÈTEMENT les filigranes, signatures d'enseignants, noms en
   arrière-plan, ou tout texte en diagonale superposé sur le contenu
   (par exemple des noms comme "MOURAD ARBI" ou tout autre nom/signature
   imprimé en fond de page). Ces éléments NE FONT PAS partie du contenu
   mathématique. Ne les transcris JAMAIS.
...
def _transcribe_with_retry(
    ...
    """Call Gemini multimodal with exponential backoff."""
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            last_err = e
            sleep_s = base_sleep * (2 ** (attempt - 1))  # exponential: 3, 6, 12
...
def main():
    ...
    # Pre-fetch existing .tex to avoid per-file existence API calls
