...
'             - Log-sheet initialization and row writing
'             - Application-state save/restore
'             - Split of a comma-delimited string
'             - Reliable last-data-row detection
'============================================================================
Option Explicit
...
' value matches headerName (case-insensitive, trimmed).
...
Public Function GetColIndex(ws As Worksheet, _
    ...
    For Each c In ws.Range(ws.Cells(headerRow, 1), ws.Cells(headerRow, lastCol))
        If LCase$(Trim$(CStr(c.Value))) = LCase$(Trim$(headerName)) Then
...
Public Function AssertColIndex(ws As Worksheet, _
    ...
    If idx = 0 Then
        Err.Raise vbObjectError + 1001, "AssertColIndex", _
                  "Column header not found: """ & headerName & _
                  """ in sheet """ & ws.Name & """"
    End If
...
' ─────────────────────────────────────────────────────────────────────────────
' SaveAppState / PauseApp / RestoreAppState
' ─────────────────────────────────────────────────────────────────────────────
...
' ─────────────────────────────────────────────────────────────────────────────
' GetLastDataRow
' Returns the last row that contains any real content/formula on the sheet.
' This is more reliable than UsedRange for sheets with old formatting.
' Returns 1 if the sheet is effectively empty.
' ─────────────────────────────────────────────────────────────────────────────
